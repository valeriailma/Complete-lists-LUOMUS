...
#if given, show.sp.names.beta should be a vector with length equalling number of models ###############!!!!!! change according to number of models!
show.sp.names.beta = c(TRUE,TRUE, TRUE)
...
if(file.exists(filename)){
  ...
  pdf(file= file.path(resultDir,"parameter_estimates6.pdf"))
  for(j in 1:nm){
    cat(c("\n",names(models)[j],"\n","\n"),file=text.file,sep="",append=TRUE)
    m = models[[j]]
    if(m$XFormula=="~."){
